...
# INTERNAL IMPORTS
# ============================================================================
from traitly.fruit_phenotyping.processing import get_fruit_contour, get_internal_pericarp_contour
...
def circular_mean_and_std_hue(hue_values: np.ndarray,
    ...
    """
    Calculate circular mean and std (degrees) for hue.
    hue_values: OpenCV hue [0,179]
    hue_degree_values: optional hue in degrees [0,360]
    """
    if hue_values is None or len(hue_values) == 0:
        return np.nan, np.nan

    if hue_degree_values is not None:
        hue_deg = hue_degree_values.astype(np.float32)
    else:
        hue_deg = hue_values.astype(np.float32) * 2.0  # [0,358]
    ...
    # Circular standard deviation (one common definition):
    R = np.sqrt(s*s + c*c)
...
def analyze_all_fruits_color(
    ...
    If renumber=True:
        - returned dict keys are sequential fruit ids (1..n)
    If renumber=False:
        - returned dict keys are original fruit ids from fruit_locule_map
    """
    ...
    # Pre-calculate a 'nan' dictionary
...
def get_single_fruit_masks(
    ...
    if fruit_id is not None:
        if not isinstance(fruit_id, int):
            raise TypeError("ERROR: fruit_id must be an integer.")
        if fruit_id < 1 or fruit_id > len(iter_map):
            raise ValueError(f"ERROR: fruit_id should be between 1 and {len(iter_map)}")
...
############################################################
# Get pixel-level color distributions for all fruits       #
############################################################

def get_fruit_color_distributions(
    img: np.ndarray,
    ...
    """
    For each fruit, compute the pixel-value histogram (distribution) of each
    color channel over the total_pericarp mask (fruit region, locules excluded).

    Color ranges (after normalization):
        RGB  → R, G, B:  0–255         (256 bins each)
        Lab  → L:        0–100         (101 bins)
                a, b:   -128–127       (256 bins each)
        HSV  → H:        0–359°        (360 bins, values land on even degrees)
               S, V:    0–100         (101 bins each)
        Gray → Gray:     0–255         (256 bins)

    Parameters
    ----------
    img           : Input BGR image.
    mask          : Binary mask (fruit=255, background=0, locules=0).
    contours      : All contours (fruit + locule contours).
    fruit_locule_map : fruit_id -> list of locule contour indices.
    image_name    : Written to every row.
    label         : Class label written to every row.
    color_space   : 'all' or comma-separated subset of {'rgb','lab','hsv','gray'}.
    renumber      : Renumber fruit IDs from 1..n if True.
    dark_threshold: Pixels with gray <= threshold are excluded.
    normalize     : If True, divide counts by n_valid_pixels (relative frequency).

    Returns
    -------
    List[Dict[str, float]] — one dict per fruit.
    """
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Define bin ranges per channel                                        #
    # ------------------------------------------------------------------ #
    # Each entry: (column_prefix, bin_start, bin_stop_inclusive)
    # bin values are integers after rounding the normalized float pixels
    ...
    if hsv_img is None:
        ...
        hsv_image = hsv_img
    # ------------------------------------------------------------------ #
    # Pre-convert full image to each color space once                      #
    # ------------------------------------------------------------------ #
    ...
    for fruit_id, locule_indices in iter_map.items():
        ...
        # ---- Lab (normalized via normalize_lab_values) ----------------- #
        if 'lab' in spaces:
        ...
        # ---- HSV (normalized via normalize_hsv_values) ----------------- #
        if 'hsv' in spaces:
...
def compute_fruit_hue_index(
    ...
    orange_hue_range: Tuple[int, int] = (21, 39),
    yellow_hue_range: Tuple[int, int] = (40, 70),
    homogeneity_threshold: float = 0.80,
) -> pd.DataFrame:
    """
    Compute red/orange/yellow color index per fruit from Hue histogram columns.

    Parameters
    ----------
    df                     : DataFrame from get_fruit_color_distributions()
    red_hue_ranges         : list of (min°, max°) for red (wraps around 0°)
    orange_hue_range       : (min°, max°) for orange
    yellow_hue_range       : (min°, max°) for yellow
    homogeneity_threshold  : dominant color ratio above this → pure category
                             else → 'mixed'
    ...
    pd.DataFrame with columns:
        image_name, fruit_id,
        red_pixels, orange_pixels, yellow_pixels, total_hue_pixels,
        red_ratio, orange_ratio, yellow_ratio,
        color_homogeneity,   # max(ratios) → 0.33 = perfectly mixed, 1.0 = uniform
        dominant_color,      # color with highest ratio
        color_category       # 'red','orange','yellow','mixed'
    """
    h_cols_all = [f'H_{i}' for i in range(360)]
    missing = [c for c in h_cols_all if c not in df.columns]
    if missing:
        raise ValueError(
            "Missing Hue columns. Run get_fruit_color_distributions() with color_space='hsv'."
        )
    ...
    for _, row in df.iterrows():
        ...
        orange_px = row[orange_cols].sum()
        yellow_px = row[yellow_cols].sum()
        total_px  = red_px + orange_px + yellow_px

        if total_px == 0:
            red_ratio = orange_ratio = yellow_ratio = np.nan
            color_homogeneity = np.nan
            dominant_color    = 'unknown'
            color_category    = 'unknown'
        else:
            red_ratio    = red_px    / total_px
            orange_ratio = orange_px / total_px
            ...
            # Minimum possible is 0.33 (perfectly even 3-way split)
            # Maximum is 1.0 (all pixels one color)

            color_category = (
                dominant_color if color_homogeneity >= homogeneity_threshold
                else 'mixed'
            )
