# # traitly/fruit_phenotyping/external_analyzer.py
...
#                     detect_color_checker: bool = False) -> None: 
...
#                     get_color_distribution: bool = False):
...
#             get_color_distribution = get_color_distribution
#         )
...
##########################################################################################
# Global worker for parallel processing (must be at module level for pickle)
##########################################################################################
...
class FruitExternalAnalyzer(FruitInternalAnalyzer): 
    ...
    def process_single_file(self, config=None, json_path=None,
        ...
        if json_path is not None and _os.path.exists(json_path):
            with open(json_path, 'r', encoding='utf-8') as f:
                resolved = _json.load(f) or {}
    ...
    def analyze_folder(
        ...
        font_scale=None, font_thickness=None, font_color=None,
        ...
        """
        import os as _os, copy as _copy, json as _json
        ...
        # Collect images
        img_paths = sorted([
            _os.path.join(folder_path, f)
            for f in _os.listdir(folder_path)
            if _Path(f).suffix.lower() in _valid_ext
        ])
        if not img_paths:
            raise ValueError(f"No valid images found in: {folder_path}")

        if output_path is None:
            output_path = _os.path.join(folder_path, "Results")
        _os.makedirs(output_path, exist_ok=True)

        # Build config from json/dict + individual params
        cfg = _copy.deepcopy(config) if config else {}
        if json_path is not None and _os.path.exists(json_path):
            with open(json_path, 'r', encoding='utf-8') as f:
                cfg.update(_json.load(f) or {})
        ...
        if verbose:
            ...
            if json_path:
                print(f"    > Passing parameters from: {json_path}\n")
        ...
        def _run_one(img_path):
            ...
            try:
                ...
                return df_m, df_c, err, n, ann_img, _os.path.basename(img_path), _t.time() - t0
            except Exception as e:
                return None, None, {
                    'filename': _os.path.basename(img_path), 'status': f'Error: {str(e)}'
                }, 0, None, _os.path.basename(img_path), _t.time() - t0

        if num_cores == 1:
            for img_path in _tqdm(img_paths, desc="Processing images", unit="img", disable=not verbose):
            ...
            with ProcessPoolExecutor(max_workers=num_cores) as executor:
                ...
                for future in _tqdm(_as_completed(futures), total=len(futures),
                    ...
                    if err:
                        ...
                        if ann_img is not None:
                            out_img = _os.path.join(output_path,
                                                    f"{_os.path.splitext(fname)[0]}_annotated.jpg")
                            _cv2.imwrite(out_img, ann_img)
                        if df_m is not None: all_morphology.append(df_m)
                        if df_c is not None: all_color.append(df_c)
                        total_fruits += n

        # Merge & save CSVs
        df_morph_all = _pd.concat(all_morphology, ignore_index=True) if all_morphology else None
        df_color_all = _pd.concat(all_color,      ignore_index=True) if all_color      else None

        morph_csv = color_csv = None
        if df_morph_all is not None:
            morph_csv = _os.path.join(output_path, "morphology_results.csv")
            df_morph_all.to_csv(morph_csv, index=False)
        if df_color_all is not None:
            color_csv = _os.path.join(output_path, "color_results.csv")
            df_color_all.to_csv(color_csv, index=False)

        # Session report
        session_end = _datetime.now()
        ...
        session_txt = _os.path.join(output_path, "session_report.txt")
        ...
        if errors:
            ...
            error_lines = [
                ...
                "", sep, header, sep,
                 ...
                 for e in errors] + [sep]
            error_txt = _os.path.join(output_path, "error_report.txt")
        ...
        if verbose:
            ...
            if morph_csv:  print(f"        - {_os.path.basename(morph_csv)}")
            if color_csv:  print(f"        - {_os.path.basename(color_csv)}")
            print(f"        - {_os.path.basename(session_txt)}")
            if error_txt:  print(f"        - {_os.path.basename(error_txt)}")
